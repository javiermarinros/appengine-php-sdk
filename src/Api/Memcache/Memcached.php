...
class Memcached
{
    ...
    private function getInternal(
        ...
        if (empty($result) && !is_null($cache_cb) && is_null($cas_token)) {
            ...
            if (!empty($result)) {
                ...
                $this->result_code = self::RES_SUCCESS;
                try {
                    $value = MemcacheUtils::deserializeValue(
                        $item->getValue(),
                        $item->getFlags()
                    );
                    if ($for_peek) {
    ...
    {
        return $this->getDelayedInternal(
            false /* $for_peek */,
            $keys,
            $with_cas,
            $value_cb
    ...
    private function getDelayedInternal(
        $for_peek,
        $keys,
        $with_cas = false,
        $value_cb = null
     ...
     *
     */
    public function peekDelayedByKey(
        $server_key,
        $keys,
        $with_cas = false,
        $value_cb = null
    ...
    private function getMultiInternal(
        $for_peek,
        $for_cas,
        $keys,
        &$cas_tokens = null,
     ...
     *
     */
    public function peekMultiByKey(
        $server_key,
        $keys,
        $with_cas = false,
        $value_cb = null
     ...
     * @return The new item's value on success or false on failure.
     */
    private function incrementInternal(
        $key,
        $offset,
        $initial_value,
        $expiry,
    ...
    public function setMulti($items, $expiration = 0)
    {
        ...
        try {
            $set_results = MemcacheUtils::setMultiWithPolicy(
                $items,
                $expiration,
                SetPolicy::SET
            );
